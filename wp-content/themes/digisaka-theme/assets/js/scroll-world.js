...
function mountScrollWorld(container, config) {
  ...
  SEGMENTS.forEach(s => {
    ...
    const img = el('img', 'sw-scene__still'); img.alt = s.alt || ''; img.decoding = 'async'; img.loading = s.si === 0 ? 'eager' : 'lazy';
  ...
  let vh = window.innerHeight, stageX = 0, totalW = 0, worldY = 0, activeIndex = -1, ticking = false;
  let laidOutW = window.innerWidth;   // width the current layout was computed at (see onResize)

  function layout() {
    vh = window.innerHeight;
    laidOutW = window.innerWidth;
    worldY = container.getBoundingClientRect().top + (window.scrollY || window.pageYOffset);
  ...
  function jumpTo(i) {
    ...
    window.scrollTo({ top: worldY + seg.start + (seg.end - seg.start) * 0.5, behavior: reduce ? 'auto' : 'smooth' });
  }
  ...
  function read() {
    ...
    const y = pageY - worldY;
    const beforeWorld = y < (-0.5 * vh);
    const afterWorld = y > ((totalW + 0.9) * vh);
    container.classList.toggle('is-world-before', beforeWorld);
    container.classList.toggle('is-world-after', afterWorld);
    ...
    for (let i = 0; i < N; i++) {
      ...
      const c = copies[i];
      c.style.opacity = cop;
      const copyShift = (0.5 - pr) * 4;
      if (isMobile()) {
        c.style.transform = reduce ? 'none' : `translateY(${copyShift}vh)`;
      } else {
        c.style.transform = reduce ? 'translateY(-50%)' : `translateY(calc(-50% + ${copyShift}vh))`;
      }
...
function injectCSS() {
  ...
  .sw-root{position:relative;overflow:clip;background:var(--sw-bg,#F5EDE0);}
  .sw-root.is-world-before .sw-sky,.sw-root.is-world-before .sw-stage,.sw-root.is-world-before .sw-copylayer,.sw-root.is-world-before .sw-route,.sw-root.is-world-before .sw-hint,.sw-root.is-world-before .sw-scrollbar,
  .sw-root.is-world-after .sw-sky,.sw-root.is-world-after .sw-stage,.sw-root.is-world-after .sw-copylayer,.sw-root.is-world-after .sw-route,.sw-root.is-world-after .sw-hint,.sw-root.is-world-after .sw-scrollbar{visibility:hidden;pointer-events:none;}
